-- Nur Academy
-- Complete Supabase SQL Editor setup
-- Includes:
-- 1. extensions
-- 2. enum types
-- 3. tables
...
begin
  ...
  values (
    ...
    case
      when coalesce(new.raw_app_meta_data ->> 'role', 'student') = 'admin' then 'admin'::public.app_role
...
grant execute on function public.protect_profile_admin_fields() to postgres, service_role;

grant select on public.courses to anon;
grant select on public.course_tracks to anon;
...
drop policy if exists enrollments_student_update_or_admin on public.enrollments;
create policy enrollments_student_update_or_admin
on public.enrollments
for update
to authenticated
using (auth.uid() is not null and (auth.uid() = user_id or public.is_admin()))
...
drop policy if exists lesson_progress_student_insert_or_admin on public.lesson_progress;
create policy lesson_progress_student_insert_or_admin
on public.lesson_progress
for insert
to authenticated
with check (auth.uid() is not null and (auth.uid() = user_id or public.is_admin()));
...
drop policy if exists course_progress_student_insert_or_admin on public.course_progress;
create policy course_progress_student_insert_or_admin
on public.course_progress
for insert
to authenticated
with check (auth.uid() is not null and (auth.uid() = user_id or public.is_admin()));
...
-- support-screenshots: <user-id>/<filename>
-- certificate-pdfs:   <user-id>/<filename>
-- =========================
drop policy if exists support_screenshots_insert_own_or_admin on storage.objects;
create policy support_screenshots_insert_own_or_admin
on storage.objects
for insert
to authenticated
with check (
...
create policy support_screenshots_select_own_or_admin
on storage.objects
for select
to authenticated
using (
...
drop policy if exists support_screenshots_update_own_or_admin on storage.objects;
create policy support_screenshots_update_own_or_admin
on storage.objects
for update
to authenticated
using (
...
create policy support_screenshots_delete_own_or_admin
on storage.objects
for delete
to authenticated
using (
...
drop policy if exists certificate_pdfs_select_own_or_admin on storage.objects;
create policy certificate_pdfs_select_own_or_admin
on storage.objects
for select
to authenticated
using (
...
drop policy if exists certificate_pdfs_insert_admin_only on storage.objects;
create policy certificate_pdfs_insert_admin_only
on storage.objects
for insert
to authenticated
with check (
...
drop policy if exists certificate_pdfs_update_admin_only on storage.objects;
create policy certificate_pdfs_update_admin_only
on storage.objects
for update
to authenticated
using (
...
drop policy if exists certificate_pdfs_delete_admin_only on storage.objects;
create policy certificate_pdfs_delete_admin_only
on storage.objects
for delete
to authenticated
using (
